...
import { createRoomWithCode, removePlayerFromRoom } from "./rooms.ts";
    ...
    socket.onmessage = (msg: MessageEvent<string>) => {
        ...
        switch (data.type) {
            ...
            case SocketIncoming.StartGame: {
                ...
                if (!room) {
                    throw new Error(`La salle n'existe pas ${socketData.roomId}.`);
                }
...
function sendMessageToRoom<TData>(roomCode: string, data: { type: SocketOutgoing; data: TData }) {
    const room = Rooms.get(roomCode);
    if (!room) {
        throw new Error(`Aucune salle n'existe avec ce code ${roomCode}.`);
    }
